...
Original file is located at
    https://colab.research.google.com/drive/1avFMP72ikTPh77Y9Ri0_a2v_KgNUKiNT
"""
...
# 파일 경로 설정
file_path_investigate = '/content/investigate.csv'

# 데이터 불러오기
investigate_data = pd.read_csv(file_path_investigate)
...
# 조사 데이터 불러오기
# investigate_data = pd.read_csv('investigate_data.csv')  # 예시로 데이터를 불러옴

# 조사 결과 데이터를 불러온 후, 날짜 포맷을 변환하여 일, 월, 년 정보를 추출
...
for p in ax1.patches:
    ...
    if height > 0:  # 빈도수가 0이 아닐 때만 레이블 추가
        ax1.text(p.get_x() + p.get_width() / 2., height + (offset - 1.5),
...
for p in ax2.patches:
    height = p.get_height()
    if height > 0:  # 빈도수가 0이 아닐 때만 레이블 추가
        ax2.text(p.get_x() + p.get_width() / 2., height + offset,
                 f'{int(height)}', ha='center', va='bottom', fontsize=10, color='black')
    else:
        p.set_visible(False)  # 빈도수가 0인 구간 숨기기
